...
export const updateArticle = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>('SELECT user_id FROM articles WHERE id = ?', [id]);
    if (rows.length === 0) {
...
export const deleteArticle = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (rows[0].user_id !== userId) {
      res.status(403).json({ error: 'Você não tem permissão para deletar este artigo.' });
      return;
    }
